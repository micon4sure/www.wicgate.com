...
3. ❌ NEVER use hardcoded scroll offsets → ✅ Use `--content-offset` for scroll alignment, `--header-height` for element positioning
   - Body/section padding: `var(--content-offset, 60px)` (mobile: 60px, desktop: 80px at md)
   - Element positioning (mobile menu, toasts): `var(--header-height)` (actual navbar height)
   - JS fallback: Use `DEFAULT_CONTENT_OFFSET` constant from `constants.ts` (60px)
...
13. ✅ ALWAYS use semantic typography tokens: `text-hero`, `text-heading`, `text-mainnav`, `text-subtab`, `text-tab`, `text-data`, `text-label`
