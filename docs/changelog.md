...
- 🐛 **Fix: Mobile Nav Viewport on iOS/Android** - Fixed Discord button and mobile nav being cut off by browser's bottom toolbar on iOS/Android: changed container height from `100vh` to `100svh` (small viewport height) which accounts for browser chrome, includes `vh` fallback for older browsers - works on iOS Safari 15.4+, Chrome 108+, Samsung Internet 21+ (Dec 26)
- 🎨 **UI: Mobile Discord Button Redesign** - Redesigned Discord link in mobile nav from full-width menu item to compact button matching desktop styling: Discord purple background with border, centered at bottom with padding, tap animation (scale 0.97 + darker background on press), removed gradient background and top/bottom borders from old design, text centered in mobile nav links - consistent button appearance across mobile and desktop (Dec 25)
