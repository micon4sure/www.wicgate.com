...
- 🎨 **Leaderboard Headers: Fixed Deep Red** - Changed leaderboard headers from gradient to solid fixed deep red (`bg-massgate-red-dark` = `#8b0000`) for darker, more subdued military aesthetic - replaced `bg-gradient-to-b from-massgate-red/95 to-massgate-red/78` with solid color - affects all leaderboards (High Scores, Total Scores, Player Leaderboard, Clan Leaderboard) - creates more serious, darker military command center feel (Oct 13)
