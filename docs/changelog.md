...
- 🎨 **Two-Tier Card Border Tokens** - Created `border-card` (25% opacity) for outer cards and `border-card-inner` (15% opacity) for nested cards in `tailwind.config.ts`: outer cards (`.widget`, `.card`, `.panel-card`, `.leaderboard-container`, `.leaderboard-panel`, `.step-card`, `.faq-item`, `.help-cta-box`) use stronger border, nested cards (`.server-group-card`, `.event-card`, `.video-item-card`, `.event-accordion-item`, `.ladder-player-item`, `.widget-stat`, `.server-item`, `.video-item`, `.player-list-item`, `.ladder-item`, `.help-item`) use subtler border, separator dividers in FAQ/Twitch cards use inline `border-white/10`, deleted unused `ScrollToTop.vue` component and CSS (~40 lines), interactive button borders kept as teal - two tokens now control all card borders site-wide (Jan 6)
