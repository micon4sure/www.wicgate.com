...
- 🎨 **Leaderboard Card Tokens** - Tokenized Statistics leaderboard card backgrounds in `tailwind.config.ts`: added `lb-row`, `lb-row-even`, `lb-header` backgroundImage tokens (steel blue gradients at 50% opacity for transparency), added `medal-text` boxShadow token for top 3 rank text shadows, updated CSS classes `.lb-row`, `.leaderboard-th`, `.lb-medal-*`, `.leaderboard-container`, `.leaderboard-panel-header` to use `theme()` references instead of hardcoded rgba values, removed unused legacy `.leaderboard-cell` and `.leaderboard-rank-cell` styles (~40 lines), `card-surface` gradient also updated to 50% opacity for consistency - single source of truth for all leaderboard styling (Jan 8)
