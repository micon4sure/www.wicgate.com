...
- 🎨 **Fluid Typography Token Restructure** - Restructured typography tokens for semantic naming and fluid scaling via CSS `clamp()`: renamed `nav-*` tokens to `tab` (unified single size 14px→18px), renamed `subtab-*` scale to `subtab` (unified single size 16px→20px), created `text-heading` (20px→26px) for ALL panel/card headers (leaderboards, dashboard cards, FAQ questions, overlays, downloads), created `text-lbdata` (16px→18px) for leaderboard data (scores, names, ranks, clan tags), created `text-mainnav` (19px→23px) for header navigation - all tokens now scale fluidly between 375px and 1280px viewports without breakpoint overrides, removed ~50 lines of responsive font-size media queries, fixed inline text class overrides in OnlinePlayersModal and YouTubeTheater header slots, converted "Network Ports" sidebar title and "Still Have Questions?" CTA header to use `text-heading` token - single source of truth for all typography scaling (Jan 9)
