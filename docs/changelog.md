...
- 📱 **Sub-Tab Breakpoint Token** - Changed sub-tabs (TabContainer, LeaderboardGroup, DynamicInfoCard, MediaEventCard) to switch between mobile dropdown and desktop horizontal at 640px (sm) instead of 1024px (lg): added `SUB_TAB_BREAKPOINT` constant in `constants.ts`, `useMobileTabs()` composable and `MobileTabDropdown` component now default to 640px, components no longer need to import/pass breakpoint explicitly - single token controls all sub-tab responsive behavior (Jan 6)
