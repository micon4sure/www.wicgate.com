...
- 🎨 **Teal Card Surfaces** - Updated card and leaderboard background colors to dark teal (#1B3031): `card-surface`, `lb-row`, `lb-row-even`, `lb-header` tokens changed from steel blue to teal gradients at 55% opacity - unified color scheme across dashboard cards, FAQ items, leaderboard panels, and Statistics tables (Jan 8)
- 🎨 **Orange Section Headers** - Replaced purple section headers with Massgate-style orange gradient: `section-header` token updated from purple (#4a1f5c/#2d1236) to rust orange/tangerine (#9E3800/#DD6804), added micro-dot mesh texture overlay (4px radial gradient) to `.server-group-header`, `.event-accordion-header`, `.faq-question-header`, `.leaderboard-header`, `.overlay-header` using `isolate` stacking context (Jan 8)
