...
- 🎨 **Glow Tokens for Deep Link Highlights** - Tokenized hardcoded rgba glow colors in `tailwind.config.ts`: added `soviet.glow-strong` (0.7 opacity), `soviet.glow-medium` (0.6), `soviet.glow-soft` (0.4) with baked-in opacity for `@keyframes` compatibility, updated both `faq-target-highlight` and `leaderboard-highlight-pulse` animations to use `theme('colors.soviet.glow-*')` tokens - single source of truth for all deep link highlight glow effects (Jan 7)
