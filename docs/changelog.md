...
- 🐛 **Fix: Mobile Discord Button Hidden on iPhone** - Fixed Discord button being hidden under browser's bottom navigation bar on iOS devices (Chrome/Safari on iPhone): added `env(safe-area-inset-bottom)` CSS environment variable to bottom padding, ensures button stays above browser chrome on notched devices (Dec 25)
